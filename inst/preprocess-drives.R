...
nds_data <- nds_data |>
  group_by(drive) |>
  arrange() |>
  mutate(gps_heading_raw = gps_heading,
         gps_heading_diff = angle_dist(gps_heading, lag(gps_heading, 1)),
         gps_heading_diff = ifelse(is.na(gps_heading_diff),
                                   0, gps_heading_diff),
         #  gps_heading_diff_dampen = ifelse(between(gps_heading_diff,-20,20),
         # gps_heading_diff, sign(gps_heading_diff)*20),
         ...
         gyro_heading_diff = ifelse(is.na(gyro_heading_diff),
                                    0, gyro_heading_diff),
